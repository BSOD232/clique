...
class AppManager {
    ...
    addApp(app, container) {
        createApp(app.imageUrl, () => {
            if (!this.expanded) {
                document.getElementById('maincontainer').style.height = app.containerHeight + 'px';
                if (document.getElementById('contentContainer')) { document.getElementById('contentContainer').remove(); };
                createAndAppend('div', null, document.getElementById('maincontainer'), 'contentContainer');
                document.getElementById('contentContainer').style.width = '500px';
                document.getElementById('contentContainer').style.height = (app.containerHeight - 60) + 'px';
                document.getElementById('contentContainer').style.position = 'absolute';
                document.getElementById('contentContainer').style.bottom = '0';
                console.log(app.htmlContent);

                var htmlString = app.htmlContent;
                var htmlDom = new DOMParser().parseFromString(htmlString, "text/html").firstChild.children[1].firstChild;
                console.log(htmlDom);
...
class CliqurParser {
    ...
    parseLine(line) {
        const createAppRegex = /^CREATE NEW APP ([A-Za-z0-9]+) IMAGE '(.+)' HEIGHT OF CONTAINER '(.+)' HTML INSIDE IS '(.+)'$/;
...
function appStoreInstall(name, img) {
    const cliqur = new CliqurParser(document.getElementById('installedappcontainer'));
    console.log(name);
...
function createLogin() {
    ...
    const container = createAndAppend('div', 'container tt5050', document.body, 'maincontainer');
    const status = createAndAppend('div', 'status', container);
    const loading = addImage('https://raw.githubusercontent.com/BSOD232/clique/main/loading.svg', null, status);
    const top = addImage('https://github.com/BSOD232/clique/blob/main/top.png?raw=true', 'top-img', container);
    const logo = addImage('https://raw.githubusercontent.com/BSOD232/clique/main/clique.png', 'logo', container);
    const login = createAndAppend('div', 'login', container);
    const loginf = createAndAppend('form', 'loginf', login);
    const username = createAndAppend('input', 'input', loginf, 'username', 'Username');
    const password = createAndAppend('input', 'input', loginf, 'password', 'Password');
    ...
    const submitBI = addImage('https://raw.githubusercontent.com/BSOD232/clique/2d7136fed5f53d1d12e5e737e53ebdeff07bbf66/arrow.svg', null, submitB);
...
function loadGUI() {
    ...
    if (username.value && password.value) {
      ...
      setTimeout(function() {
        ...
        container.classList.remove('tt5050');
        container.classList.add('tt50');
        ...
        container.appendChild(installedappcontainer);
        ...
        createAndAppend('img', null, appcontainer).src = 'https://raw.githubusercontent.com/BSOD232/clique/2d7136fed5f53d1d12e5e737e53ebdeff07bbf66/breakline.svg';
